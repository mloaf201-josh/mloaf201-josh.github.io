import http.server,socket,urllib.request,json,subprocess
...
.grid{display:grid;gap:0.5rem;margin-bottom:0.5rem}
...
.card h3{font-size:0.7rem;color:#1da1f2;margin-bottom:0.3rem;text-transform:uppercase;letter-spacing:0.5px}
.card p{font-size:0.65rem;color:rgba(255,255,255,0.2);margin-bottom:0.4rem}
.card input{width:100%;padding:0.4rem;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:4px;color:#e0e0ee;margin-bottom:0.3rem;font-size:0.75rem}
...
.res{background:rgba(0,0,0,0.25);border-radius:6px;padding:0.6rem;font-size:0.75rem;margin-top:0.4rem;min-height:2rem;white-space:pre-wrap;font-family:monospace;color:rgba(255,255,255,0.6);max-height:300px;overflow:auto}
.tag{font-size:0.5rem;padding:0.1rem 0.3rem;border-radius:3px;margin-left:0.3rem}
.tag.new{background:rgba(74,222,128,0.1);color:#4ade80}
...
<div class="grid c3">
<div class=card style="border-color:rgba(74,222,128,0.15)"><h3>&#x1F50D; Sherlock <span class="tag new">NEW</span></h3><p>Find username across 400+ sites</p><input id=t5 placeholder=username value=johndoe><button onclick=g(5)>Search</button><div class=res id=r5></div></div>
<div class=card style="border-color:rgba(74,222,128,0.15)"><h3>&#x1F4E7; Holehe <span class="tag new">NEW</span></h3><p>Check email on websites</p><input id=t6 placeholder=email@example.com><button onclick=g(6)>Check</button><div class=res id=r6></div></div>
<div class=card style="border-color:rgba(74,222,128,0.15)"><h3>&#x1F9F1; Maigret <span class="tag new">NEW</span></h3><p>Advanced username search</p><input id=t7 placeholder=username><button onclick=g(7)>Search</button><div class=res id=r7></div></div>
</div>
    ...
    def do_GET(self):
        ...
        try:
            if t=="1":ip=socket.gethostbyname(qry);r="Target: "+qry+"\nIP: "+ip
                ...
                v4=list(set(i[4][0] for i in a if i[0]==socket.AF_INET));v6=list(set(i[4][0] for i in a if i[0]==socket.AF_INET6))
                r="IPv4: "+(", ".join(v4) or "none")+"\nIPv6: "+(", ".join(v6) or "none")
            elif t=="5":
                if qry:
                    p=subprocess.run(["sherlock",qry,"--output","/tmp/sherlock.txt"],capture_output=True,text=True,timeout=30)
                    o=p.stdout[:2000] or p.stderr[:200]
                    if os.path.exists("/tmp/sherlock.txt"):
                        with open("/tmp/sherlock.txt")as f:o=f.read()[:2000]
                    r=o or "No results found"
            elif t=="6":
                if qry:
                    p=subprocess.run(["holehe",qry],capture_output=True,text=True,timeout=30)
                    r=p.stdout[:2000] or p.stderr[:200]
            elif t=="7":
                if qry:
                    p=subprocess.run(["maigret",qry,"--all", "--timeout","15"],capture_output=True,text=True,timeout=45)
                    r=p.stdout[:2000] or p.stderr[:200]
        except subprocess.TimeoutExpired:r="Command timed out"
