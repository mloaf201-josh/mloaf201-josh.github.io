"""OSINT v7 — Auto-installs dependencies on first run"""
import subprocess, os, sys

# Auto-install missing packages on first run
DEPS = ["sherlock-project", "holehe"]
for dep in DEPS:
    subprocess.run(["pip3", "install", "-q", dep, "--timeout", "60"],
                   capture_output=True, timeout=120)

# Remove the auto-install code so it doesn't run every restart
# (pip will skip if already installed)

import http.server,socket,urllib.request,json,os
...
<h3>&#x1F50D; Sherlock <span class="tag n">NEW</span></h3><p>Find username across 400+ sites</p>
<div class=opts><label><input type=checkbox id=c5> Print all</label><label><input type=checkbox id=c5b> NSFW</label></div>
<input id=t5 placeholder=username value=johndoe><button onclick=g(5)>Search</button><div class=res id=r5></div></div>
<div class=card style="border-color:rgba(74,222,128,0.15)">
<h3>&#x1F4E7; Holehe <span class="tag n">NEW</span></h3><p>Check email registration on 120+ sites</p>
<div class=opts><label><input type=checkbox id=c6> Only used</label><label><input type=checkbox id=c6b> Skip recovery</label></div>
<input id=t6 placeholder=email@example.com><button onclick=g(6)>Check</button><div class=res id=r6></div></div>
<div class=card style="border-color:rgba(74,222,128,0.15)">
<h3>&#x1F9F1; Maigret <span class="tag d">BETA</span></h3><p>Advanced username search (if installed)</p>
<div class=opts><label><input type=checkbox id=c7> All sites</label></div>
<input id=t7 placeholder=username><button onclick=g(7)>Search</button><div class=res id=r7></div></div>
</div>
    ...
    def do_GET(self):
        ...
        try:
            if t=="1":ip=socket.gethostbyname(qry);r="Target: "+qry+"\nIP: "+ip
                ...
                cmd=["python3","-m","sherlock_project",qry,"--timeout","30"]
                if o1:cmd+=["--print-all"]
                if o2:cmd+=["--nsfw"]
